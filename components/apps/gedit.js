...
export class Gedit extends Component {
    ...
    sendMessage = async () => {
        ...
        if (name.length === 0) {
            $("#sender-name").val('');
            $("#sender-name").attr("placeholder", "Name / Email must not be empty!");
            error = true;
        }
        ...
        if (serviceID && templateID) {
            ...
            window.open(`mailto:yashbansal531@gmail.com?subject=${encodeURIComponent(subject || 'Message from Portfolio')}&body=${encodeURIComponent(`From: ${name}\n\n${message}`)}`);
            this.setState({ sending: false });
            setTimeout(() => {
                $("#close-gedit").trigger("click");
            }, 300);
        ...
        return (
            <div className="w-full h-full relative flex flex-col bg-ub-cool-grey text-white select-none">
                <div className="flex items-center justify-between w-full bg-ub-gedit-light bg-opacity-60 border-b border-t border-blue-400 text-sm">
                    <span className="font-bold ml-2">Send a Message to Yash</span>
                    <div className="flex">
                ...
                <div className="relative flex-grow flex flex-col bg-ub-gedit-dark font-normal windowMainScreen">
                    ...
                    <div className="relative">
                        <input id="sender-subject" className=" w-full my-1 text-ubt-gedit-blue focus:bg-ub-gedit-light gedit-subject outline-none text-sm font-normal pl-6 py-0.5 bg-transparent" placeholder="Subject (e.g. Software Engineering Opportunity / Project)" spellCheck="false" autoComplete="off" type="text" />
                        <span className="absolute left-1 top-1/2 transform -translate-y-1/2 font-bold text-sm text-ubt-gedit-blue">2</span>
                    </div>
                    <div className="relative flex-grow">
                        <textarea id="sender-message" className=" w-full gedit-message font-light text-sm resize-none h-full windowMainScreen outline-none tracking-wider pl-6 py-1 bg-transparent" placeholder="Type your message here..." spellCheck="false" autoComplete="none" type="text" />
